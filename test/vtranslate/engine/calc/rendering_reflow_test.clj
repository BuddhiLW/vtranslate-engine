...
(deftest an-omitted-unit-stays-out-of-a-reflowed-track-too
  (let [credit [6000 8000 "Legendas pela comunidade Amara.org"]
        tc     (->tcues "pt-BR" (conj fragments credit))
        tc     (update tc :units (fn [us] (conj (vec (butlast us))
                                                (assoc (last us) :segment/omit? true))))]
    (doseq [rules [{:strategy :optimal} {:strategy :greedy :max-chars-line 42}]]
      (testing (str (:strategy rules))
        (let [out (cues (sut/build-subtitle-track tc (assoc spec :reflow rules)))]
          (is (seq out))
          (is (not-any? #(re-find #"Amara" %) (mapcat :lines out)))
          (is (= (range 1 (inc (count out))) (map :index out))))))))
