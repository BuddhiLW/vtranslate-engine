...
(defn build-subtitle-track
  "Turn a TranslatedCues aggregate into a render-ready SubtitleTrack: make the
   ...
   With `:reflow` rules the remaining units are first shaped as a whole track by
   the strategy the rules name (calc.reflow): lines broken for the TARGET
   language, which is only known here, and timing weighed across neighbours.
   Without them every unit is one cue with one line, as it always was.
   => (r/ok SubtitleTrack) | (r/err :error/render-failed ...)."
  [translated-cues {:keys [id format reflow]}]
  (let [units (remove :segment/omit? (:units translated-cues))]
