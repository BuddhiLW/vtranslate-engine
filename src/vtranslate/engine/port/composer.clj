...
(defprotocol IVideoComposer
  ...
  (compose [this video-source subtitle-track opts]
    "=> (r/ok {:output-uri s}) for a single-output composition, or
     (r/ok {:output-uris {variant-key path}}) for a multi-output one (e.g. :both)
     | (r/err :error/compose-failed {:reason s}).
