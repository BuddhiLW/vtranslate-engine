(ns vtranslate.engine.adapters.segmenter.silero-vad-native
  ...
  (:import [ai.onnxruntime OnnxTensor OrtEnvironment OrtSession OrtSession$SessionOptions]
           [java.util HashMap])
  (:require [vtranslate.engine.port.native-call :as p.native]
...
(defn speech-probs
  "Return vector of speech probabilities, one per Silero window.
   ...
   The whole session (creation, every window, teardown) runs on a thread with an
   explicitly sized stack: ONNX Runtime 1.29 and later recurse deeply while
   optimizing the graph and overflow the JVM's default stack IN NATIVE CODE,
   which is a SIGSEGV that kills the process rather than an exception anyone can
   catch. hive-weave.stack owns that thread; see its docstring for the
   measurement, and pass :stack-bytes to any pool that runs native work."
